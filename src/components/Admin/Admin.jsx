...
const Admin = memo(function Admin() {
  ...
  const sectorChartData = Object.entries(sectores)
    .map(([name, value]) => ({
      ...
      value,
      beneficiarios: sectoresPersonas[name] || value,
    }))
    ...
    if (active && payload?.length) {
      return (
        <div className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs shadow-lg">
          ...
          <p className="text-gray-600">Asociadas: <span className="font-medium">{payload[0].value}</span></p>
          {payload[0].payload.beneficiarios && (
            <p className="text-gray-600">Beneficiarios: <span className="font-medium">{payload[0].payload.beneficiarios}</span></p>
          )}
        </div>
      );
    }
    ...
    <div className="space-y-6">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {sectorChartData.map((item, i) => {
                ...
                const maxVal = Math.max(...sectorChartData.map((d) => d.value));
                const pct = (item.value / maxVal) * 100;
                return (
                  <tr key={i} className="transition-colors duration-150 hover:bg-blue-50/50">
                    <td className="px-4 py-2.5 text-sm text-gray-400">{i + 1}</td>
                    <td className="px-4 py-2.5 text-sm font-medium text-gray-900 whitespace-nowrap">{item.name}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-700 font-semibold">{item.value}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{item.beneficiarios}</td>
                    <td className="px-4 py-2.5 text-sm text-gray-600">{prom}</td>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Total Sectores</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{Object.keys(sectores).length}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Total Beneficiarios</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{totalBeneficiarios}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Prom. Personas/Hogar</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{promPersonas}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-400">Rango de Edad</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{edadMin} - {edadMax}</p>
        </div>
      </div>
    </div>
  );
